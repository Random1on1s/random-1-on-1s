...
class Random1on1BotConfig(object):
    ...
    def __post_init__(self):
        validate_announcement_prefs(
            **{
                'guild_id': self.guild_id,
                'dm_matches': self.dm_matches,
                'announce_matches': self.announce_matches
            })
...
def validate_announcement_prefs(**dictionary):
    ...
    if "dm_matches" in dictionary and "announce_matches" in dictionary:
        ...
        if dictionary["announce_matches"] == False and dictionary[
                "dm_matches"] == False:
            raise ValueError(
                "announce_matches and dm_matches cannot both be false")
...
def config_from_dict(dictionary) -> Random1on1BotConfig:
    ...
    return Random1on1BotConfig(
        ...
        announcement_channel=dictionary.get("announcement_channel",
                                            DEFAULT_ANNOUNCEMENT_CHANNEL),
        history_channel=dictionary.get("history_channel",
                                       DEFAULT_HISTORY_CHANNEL),
        logging_channel=dictionary.get("logging_channel",
                                       DEFAULT_LOGGING_CHANNEL),
        announce_matches=dictionary.get("announce_matches",
                                        DEFAULT_ANNOUNCE_MATCHES),
